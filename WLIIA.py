...
def read_spectrum(file_name) -> pd.DataFrame:
    """ 
    Loads a spectrum from a .spectrum or .fits file and returns it as a pandas DataFrame with columns 
    'Wavelength' and 'Spectrum'. Returns a DataFrame with the spectrum data or a value error if the file format is unsupported.

    Parameters
    ----------
    file_name : str
        The path to the spectrum file. It can be either a .spectrum or .fits file.

    Returns
    -------
    pd.DataFrame
        A DataFrame containing the spectrum data with columns 'Wavelength' and 'Spectrum'.
    ...
    if file_name.endswith('.fits'): 
        with fits.open(file_name) as hdul:
            ...
            # Change the column names to Wavelength and Spectrum
            df.columns = ['Wavelength', 'Spectrum']
            return df

    else:
        raise ValueError("Unsupported file format. Please provide a .spectrum file.")
    
def clean_spectrum(df, plot = False) -> pd.DataFrame:
    """
    Cleans the spectrum DataFrame by removing rows with NaN values and ensuring the data types are correct.

    Parameters
    ----------
    df : pd.DataFrame
        The input DataFrame containing the spectrum data.

    Returns
    -------
    pd.DataFrame
        A cleaned DataFrame with no NaN values and correct data types.
    """
    clean_df = df[df['Spectrum'] > 0]

    if plot:
        plt.figure(figsize=(10, 6))
        plt.plot(clean_df['Wavelength'], clean_df['Spectrum'], label='Cleaned')
...
def build_line_dict_ESPRESSO(spectral_type, min_intensity=0, top_n=None) -> dict:
        """
        Builds a dictionary of ESPRESSO lines for a given spectral type.

        Parameters
        ----------
        spectral_type : str
            The spectral type for which to build the line dictionary.
        min_intensity : float, optional
            The minimum intensity threshold for including lines (default is 0).
        top_n : int, optional
            The maximum number of lines to include (default is None, meaning all lines).

        Returns
        -------
        dict
            A dictionary containing the ESPRESSO lines for the specified spectral type.
        """
        data_file = f"./data/str{spectral_type}_espresso.txt"

        if data_file: 
            print(f"Loading ESPRESSO line data for spectral type {spectral_type} from {data_file}")
        #read in 
        # Pseudo code for Pui Hin
...
def match_known_lines(wavelengths, flux, line_dict, tolerance=0.01, prominence=0.2, region=None) -> list: 
    """
    Match observed spectrum lines to known lines from a line dictionary.
    ...
    line_dict : dict
        A dictionary of known lines.
    tolerance : float, optional
        The maximum allowed difference between observed and known lines (default is 0.01).
    prominence : float, optional
        The minimum prominence of peaks to consider (default is 0.2).
    region : tuple, optional
        A tuple specifying the wavelength range to consider (default is None, meaning all wavelengths).

    Returns
    -------
    list
        A list of matched lines.
    """
    inverted = 1 - flux
